# a1-04.py Muestra el número de capas y parámetros del modelo GSP-2. Versión modificada de p03_train.py
...
# Import the architecture and configurations from P02_gpt_model.py
# from p02_gpt_model import GPTModel, GPT_CONFIG_355M


# Point Python to the folder one level up, named 'gsp-2' so "from p02_gpt_model and from config" work
import_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../gsp-2'))
...
if __name__ == "__main__":
    ...
    console.print(f"Total number of trainable model parameters: {num_params:,}", style="blue")
    print()


    # Ahora muestro el número de parámetros de cada capa
    num_layers = len(model.trf_blocks)
    console.print(f"Number of Transformer layers: {num_layers}", style="blue")

    # Iterate through the actual transformer blocks to count their parameters
    total_trf_params = 0
    for i, block in enumerate(model.trf_blocks):
        block_params = sum(p.numel() for p in block.parameters() if p.requires_grad)
        print(f"  - Transformer Block {i} parameters: {block_params:,}")
        total_trf_params += block_params
        
    console.print(f"\nTotal parameters inside all Transformer blocks: {total_trf_params:,}\n", style="blue")
